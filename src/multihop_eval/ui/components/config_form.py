...
def _arango_form(prefill: ArangoConfig | None) -> dict[str, Any]:
    ...
    project_name = st.text_input(
        "Autograph project name",
        value=(prefill.project_name if prefill else DEFAULT_PROJECT_NAME),
        help=(
            "Name of your Autograph project. Every collection below is derived from "
            "this as `<project>_<suffix>` (and `qa_pairs_<project>_v1` for the QA "
            "output), so setting it once propagates the name everywhere. Expand the "
            "section below only to override an individual collection name."
        ),
    )

    # Autograph collection names all share the project name; derive the defaults
    # from whatever the user just typed so the expander reflects it live. The
    # widget `key`s embed the project name so changing it refreshes the derived
    # values (while still letting a saved prefill restore explicit overrides).
    safe_project = project_name.strip() or DEFAULT_PROJECT_NAME
    derived = default_collection_names(safe_project)
    use_prefill = prefill is not None and prefill.project_name.strip() == project_name.strip()

    def _collection_default(field: str) -> str:
        if use_prefill:
            return getattr(prefill, field)
        return derived[field]

    with st.expander("Collection names (override only if your dataset differs)", expanded=False):
        st.caption(
            f"Derived from project **{safe_project}**. Edit a field to override just "
            "that collection; change the project name above to re-propagate the rest."
        )
        cols = st.columns(2)
        sim = cols[0].text_input(
            "Similarity collection",
            value=_collection_default("similarity_collection"),
            key=f"sim_collection_{safe_project}",
            help=(
        ...
        rel = cols[1].text_input(
            ...
            value=_collection_default("relations_collection"),
            key=f"rel_collection_{safe_project}",
            help=(
        ...
        rags = cols[0].text_input(
            "RAGs collection",
            value=_collection_default("rags_collection"),
            key=f"rags_collection_{safe_project}",
            help=(
        ...
        sources = cols[1].text_input(
            "Sources collection",
            value=_collection_default("sources_collection"),
            key=f"sources_collection_{safe_project}",
            help=(
        ...
        domains = cols[0].text_input(
            "Domains collection",
            value=_collection_default("domains_collection"),
            key=f"domains_collection_{safe_project}",
            help=(
        ...
        qa = cols[1].text_input(
            ...
            value=_collection_default("qa_collection"),
            key=f"qa_collection_{safe_project}",
            help=(
